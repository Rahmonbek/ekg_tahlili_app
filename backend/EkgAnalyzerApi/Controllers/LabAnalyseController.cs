...
public class LabAnalyseController : ControllerBase
{
    ...
    public LabAnalyseController(MedDataDB context, LabAnalyseService labService, PythonApiProxyService proxyService, AnalysisProgressTracker progressTracker, ICurrentUser currentUser)
    {
        _context = context;
    ...
    public async Task<IActionResult> GetById(int id)
    {
        ...
        var result = await _labService.GetLabAnalyseByIdAsync(id, clinicId.Value);
    ...
    /// POST api/lab-analyses/replace-file   (multipart: id, file, age, gender, lang)
    ...
    public async Task<IActionResult> ReplaceFile()
    {
        ...
        var belongsToClinic = await _context.LabAnalyse
        ...
        try
        {
            var response = await _proxyService.ProxyMultipartAsync("/lab/replace-file", Request, token);
            var result = await ProxyHttpResponseMapper.ToContentResultAsync(response);
            if (response.IsSuccessStatusCode && !string.IsNullOrWhiteSpace(result.Content))
                TrackAnalysisProgress(result.Content, "lab", "lab_id");
            return result;
    ...
    /// POST api/lab-analyses/retry   (multipart: id, age, gender, lang)
    ///
    ...
    public async Task<IActionResult> Retry()
    {
        ...
        try
        {
            var response = await _proxyService.ProxyMultipartAsync("/lab/retry", Request, token);
            var result = await ProxyHttpResponseMapper.ToContentResultAsync(response);
            if (response.IsSuccessStatusCode && !string.IsNullOrWhiteSpace(result.Content))
                TrackAnalysisProgress(result.Content, "lab", "lab_id");
            return result;
