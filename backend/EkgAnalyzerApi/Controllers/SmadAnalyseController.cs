...
public class SmadAnalyseController : ControllerBase
{
    ...
    public SmadAnalyseController(MedDataDB context, SmadAnalyseService smadService, PythonApiProxyService proxyService, AnalysisProgressTracker progressTracker, ICurrentUser currentUser)
    {
        _context = context;
    ...
    public async Task<IActionResult> GetById(int id)
    {
        ...
        var result = await _smadService.GetSmadAnalyseByIdAsync(id, clinicId.Value);
    ...
    /// POST api/smad-analyses/replace-file   (multipart: id, file, age, gender, lang)
    ...
    public async Task<IActionResult> ReplaceFile()
    {
        ...
        var belongsToClinic = await _context.SmadAnalyses
        ...
        try
        {
            var response = await _proxyService.ProxyMultipartAsync("/smad/replace-file", Request, token);
            var result = await ProxyHttpResponseMapper.ToContentResultAsync(response);
            if (response.IsSuccessStatusCode && !string.IsNullOrWhiteSpace(result.Content))
                TrackAnalysisProgress(result.Content, "smad", "smad_id");
            return result;
    ...
    /// POST api/smad-analyses/retry   (multipart: id, age, gender, lang)
    ///
    ...
    public async Task<IActionResult> Retry()
    {
        ...
        try
        {
            var response = await _proxyService.ProxyMultipartAsync("/smad/retry", Request, token);
            var result = await ProxyHttpResponseMapper.ToContentResultAsync(response);
            if (response.IsSuccessStatusCode && !string.IsNullOrWhiteSpace(result.Content))
                TrackAnalysisProgress(result.Content, "smad", "smad_id");
            return result;
