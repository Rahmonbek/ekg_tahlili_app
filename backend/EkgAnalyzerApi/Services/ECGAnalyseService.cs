    ...
    public class ECGAnalyseService
    {
            ...
            bool? hasDiagnosis = null)
        {
            ...
            if (hasDiagnosis.HasValue)
            {
                ...
                {
                    query = query.Where(e => _context.AnalysisDiagnoses.Any(d => d.AnalysisType == "ecg" && d.AnalysisId == e.Id));
                }
                else
                {
                    query = query.Where(e => !_context.AnalysisDiagnoses.Any(d => d.AnalysisType == "ecg" && d.AnalysisId == e.Id));
            ...
            var items = await query
                ...
                .Select(e => new ECGAnalyseListDTO
                {
                    Id              = e.Id,
                    DocumentNumber = e.DocumentNumber,
                    ThumbnailUrl = e.GeneratedShortFileLink,
                    Status          = e.Status,
                    CreatedAt = e.CreatedAt,
                    AnalysisDate = e.AnalysisDate,
                    Patcient = new PatcientForECG
                    ...
                    AIAnswerData = e.AIAnswerData,
                    AiLang = e.AiLang,
                    HasDiagnosis = _context.AnalysisDiagnoses.Any(d => d.AnalysisType == "ecg" && d.AnalysisId == e.Id)
                })
                .ToListAsync();
        ...
        public async Task<ECGAnalyseDTO?> GetECGAnalyseByIdAsync(int id, int? clinicId = null)
        {
        ...
        public async Task<PagedResult<ECGAnalyseListDTO>> GetECGAnalysesByDoctorAsync(
            int doctorId,
            int page = 1,
            int pageSize = 10,
            string? search = null,
        ...
        public async Task<int> GetUnviewedECGCountByDoctorAsync(int doctorId)
        {
            return await _context.ECGAnalyseDoctor
                .Where(d => d.DoctorId == doctorId && !d.IsViewed)
                .CountAsync();
        }

        public async Task MarkECGViewedByDoctorAsync(int doctorId)
        {
            var rows = await _context.ECGAnalyseDoctor
                .Where(d => d.DoctorId == doctorId && !d.IsViewed)
                .ToListAsync();
