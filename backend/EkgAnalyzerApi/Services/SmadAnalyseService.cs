    ...
    public class SmadAnalyseService
    {
            ...
            bool? hasDiagnosis = null)
        {
            ...
            if (hasDiagnosis.HasValue)
            {
                ...
                {
                    query = query.Where(e => _context.AnalysisDiagnoses.Any(d => d.AnalysisType == "smad" && d.AnalysisId == e.Id));
                }
                else
                {
                    query = query.Where(e => !_context.AnalysisDiagnoses.Any(d => d.AnalysisType == "smad" && d.AnalysisId == e.Id));
        ...
        public async Task<PagedResult<SmadAnalyseListDTO>> GetSmadAnalysesByNurseAsync(
            int doctorId,
            int page = 1,
            int pageSize = 10,
            string? search = null,
