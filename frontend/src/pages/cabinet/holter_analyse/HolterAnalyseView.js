...
import { replaceHolterFile } from '../../../host/holterService';
import useDocumentTitle from '../../../tools/useDocumentTitle';
import AiLanguageNotice from '../../../components/shared/AiLanguageNotice';

const { Title, Text } = Typography;

export default function HolterAnalyseView() {
    const { id } = useParams();
    ...
    useDocumentTitle(
        data
            ? `${[data.patcient?.lastName, data.patcient?.firstName].filter(Boolean).join(' ')} — ${data.documentNumber || t('analyse_holter', { defaultValue: 'Holter' })}`
            : t('analyse_holter', { defaultValue: 'Holter' })
    )
    ...
    const getData = useCallback(async () => {
        ...
        setloader(true);
        try {
            const res = await get_holter_analyse_by_id(id);
            setData(res.data);
        } catch (err) {
            navigate('/holter-analyses');
        } finally {
            setLoading(false);
            setloader(false);
        }
    ...
    return (
        <div>
            <AnalyseViewHeader
                t={t}
                onBack={() => navigate('/holter-analyses')}
                downloadNode={data.status === 2 ? <DownloadReportButton type="holter" id={data.id} size="middle" className="analysis-view-download-btn" /> : null}
                clinic={clinic}
            ...
            <HolterOldResult data={data} initialOpen={true} showMeta={false} />

            <DoctorDiagnosisBlock analysisType="holter" analysisId={data.id} />
