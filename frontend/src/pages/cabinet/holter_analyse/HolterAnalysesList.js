...
import { get_holter_analyses_by_clinic, get_holter_analyses_by_doctor, get_holter_analyses_by_nurse, mark_holter_viewed } from '../../../host/requests/HolterAnalyseRequest';
import { formatDateTime, formatDate, calculateAge , rowNumber } from '../../../tools/formatters';
...
export default function HolterAnalysesList() {
    // Qo'llanma qadamlari ro'yxatdan o'tkaziladi; tugma header'da
    usePageTour(analysisListTour);
    const { t } = useTranslation()
    useDocumentTitle(t('analyse_holter', { defaultValue: "Holter tahlillari" }));
    const navigate = useNavigate();
    const { user, setholter_unread } = useStore();
    ...
    const fetchData = useCallback(async (p) => {
        ...
        try {
            ...
            const res = isDoctor
                ? await get_holter_analyses_by_doctor(params)
                : isNurse
                    ? await get_holter_analyses_by_nurse(params)
                    : await get_holter_analyses_by_clinic(params);
    ...
    useEffect(() => {
        fetchData(page);
        if (isDoctor) {
            mark_holter_viewed().then(() => setholter_unread(0)).catch(() => { });
        }
                ...
                <Space size={0} data-tour="analysis-delete" onClick={(e) => e.stopPropagation()}>
                    <Tooltip title={t('view')}>
                        <span
                            ...
                            onClick={() => navigate(`/holter-analyses/view/${row.id}`)}
                        >
                            <FaEye />
                        </span>
                    </Tooltip>
    ...
    return (
        <div>
            <div className="main_card">
                <h1>
                    <span>
                        {t('analyse_holter', { defaultValue: 'Holter Tahlillar' })}
                    </span>
                    <button
                        onClick={() => navigate('/analyse-holter')}
                        className="btn_form" data-tour="analysis-new"
                        style={{ width: 'auto', padding: '0 24px', marginTop: 0 }}
                    >
                        {t('create_new_holter_analyse', { defaultValue: 'Yangi Holter tahlil' })}
                    </button>
                </h1>
                            ...
                            locale={{
                                emptyText: (
                                    ...
                                    activeFilterCount > 0 ? (
                                        ...
                                        <EmptyState
                                            icon={<FaHeartbeat />}
                                            message={t('no_holter_analyses', { defaultValue: 'Hech qanday Holter tahlil topilmadi' })}
                                            actionLabel={t('new_holter_analyse', { defaultValue: 'Yangi Holter tahlil' })}
                                            actionPath="/analyse-holter"
                                        />
                                    )
                                )
                            }}
