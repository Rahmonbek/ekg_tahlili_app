...
export default function HolterAnalysesList() {
    ...
    const fetchData = useCallback(async (p, s, st, dr, ab) => {
        ...
        try {
            ...
            if (items.length > 0) {
                check_has_diagnosis('holter', items.map(i => i.id))
    ...
    return (
        <div>
            <div className="main_card">
                <h1>
                    {t('analyse_holter') || 'Holter Tahlillar'}
                    <button
                        onClick={() => navigate('/analyse-holter')}
                        className="btn_form"
                        style={{ width: 'auto', padding: '0 24px', marginTop: 0 }}
                    >
                        {t('create_new_holter_analyse') || 'Yangi Holter tahlil'}
                    </button>
                </h1>
                        ...
                        <Row gutter={[12, 12]} align="bottom">
                            ...
                            <Col xs={24} sm={12} md={4}>
                                <div style={{ display: 'flex', gap: 8, height: '48px' }}>
                                    ...
                                    <button onClick={() => navigate('/analyse-holter')} className="btn_form" style={{ width: '48px', flexShrink: 0, margin: 0, height: '48px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                        <FaPlus />
                                    </button>
                                </div>
                                {hasActiveFilters && (
                        ...
                        <Table
                            ...
                            locale={{
                                emptyText: (
                                    <EmptyState
                                        icon={<FaHeartbeat />}
                                        message={t('no_holter_analyses') || 'Hech qanday Holter tahlil topilmadi'}
                                        actionLabel={t('new_holter_analyse') || 'Yangi Holter tahlil'}
