...
import { analyzeHolterFile } from '../../../host/holterService';
...
const changePositions=(val)=>{
 ...
 const getDoctorsOfClinic=async()=>{
  ...
  const getOldECGAnaylses=async(id, type)=>{
    try{
      setold_loading(true)
         var res=await get_holter_analyses_by_patcient_id({id:id, page:type=="first"?1:page})
         if(type=="first"){
          setpage(2)
 ...
 const handleSubmit = async () => {
    ...
    try {
      ...
      var res
          ...
          res = await analyzeHolterFile(formData);
      console.log(res)
      let parsedResult;
     try {
  ...
  />
     ...
     <div className='save-pat-data'>
      ...
      {check_ecg?<div className="main_card">
        <h1>{t('holter_analyse')} <Tooltip placement="bottomRight" title={t("alert_ecg")}>
                <span className='alert_icon'><IoAlertCircleSharp /></span>
            </Tooltip></h1>
        <div className="main_card_content">
              <Form
        ...
        <div className="main_card_content">
          ...
          {t("retry_holter_analyse")}
            ...
            {old_anylyses.map((item, key)=>{
              return <HolterOldResult data={item}/>
            })}
