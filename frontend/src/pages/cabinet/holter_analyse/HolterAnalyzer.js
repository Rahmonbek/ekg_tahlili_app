...
export default function HolterAnalyzer() {
    ...
    const getOldAnalyses = useCallback(async (id, type) => {
        ...
        try {
            const res = await get_holter_analyses_by_patcient_id({
    ...
    const handleSubmit = useCallback(async () => {
        ...
        try {
            ...
            const res = await analyzeHolterFile(formData);
            let parsedResult;
            try {
    ...
    return (
        <div>
                ...
                <div className="main_card">
                    ...
                    <div className="main_card_content">
                        {state.loading3 ? (
                            ...
                            <>
                                <HolterResult error={state.error} result={state.result} image={state.image} />
                                <br />
                                <Row>
                                    <Col lg={9} md={24}></Col>
                                    <Col lg={6} md={24}>
            ...
            {state.oldAnalyses.map((item) => (
                <HolterOldResult key={item.id} data={item} />
            ))}
