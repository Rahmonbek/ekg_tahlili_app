...
import { replaceSmadFile } from '../../../host/smadService';
...
export default function SmadAnalyseView() {
    ...
    useDocumentTitle(
        data
            ? `${[data.patcient?.lastName, data.patcient?.firstName].filter(Boolean).join(' ')} — ${data.documentNumber || t('analyse_smad', { defaultValue: 'SMAD' })}`
            : t('analyse_smad', { defaultValue: 'SMAD' })
    )
    ...
    return (
        <div>
            ...
            <SmadOldResult data={data} initialOpen={true} showMeta={false} />
