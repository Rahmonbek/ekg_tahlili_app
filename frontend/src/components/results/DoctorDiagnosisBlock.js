...
export default function DoctorDiagnosisBlock({ analysisType, analysisId }) {
    ...
    const handleSaveNew = async () => {
        ...
        try {
            ...
            setNewText('');
            notification.success({ message: t('success', { defaultValue: 'Muvaffaqiyatli saqlandi' }) });
        } catch (err) {
            if (err.response && err.response.status === 403) {
                notification.error({ message: t('access_denied', { defaultValue: 'Siz ushbu tahlilga biriktirilmagansiz!' }) });
            }
        } finally {
            setIsSaving(false);
        }
    ...
    const handleUpdate = async (id) => {
        ...
        try {
            ...
            setEditingId(null);
            notification.success({ message: t('success', { defaultValue: 'Muvaffaqiyatli yangilandi' }) });
        } catch (err) {
            if (err.response && err.response.status === 403) {
                notification.error({ message: t('access_denied', { defaultValue: 'Faqat o\'zingiz yozgan tashxisni tahrirlashingiz mumkin' }) });
            }
        } finally {
            setIsSaving(false);
        }
    ...
    const handleDelete = async (id) => {
        try {
            ...
            notification.success({ message: t('success', { defaultValue: 'Muvaffaqiyatli o\'chirildi' }) });
        } catch (err) {
            if (err.response && err.response.status === 403) {
                notification.error({ message: t('access_denied', { defaultValue: 'Faqat o\'zingiz yozgan tashxisni o\'chirishingiz mumkin' }) });
            }
        }
    };
    ...
    return (
        <div data-tour="view-conclusion" style={{
            background: '#fff',
            borderRadius: '12px',
            ...
            <div style={{
                ...
                <div style={{
                    width: 36, height: 36, borderRadius: '8px', background: '#E6F7F4',
                    display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#00B39A', fontSize: 18
                }}>
                    <FaUserMd />
                </div>
                <h3 style={{ margin: 0, color: '#1e293b', fontSize: '18px', fontWeight: '600' }}>
                    {t('doctor_diagnosis_title', { defaultValue: 'Shifokor Tashxisi' })}
                </h3>
            </div>
            ...
            <div style={{ padding: '20px' }}>
                {diagnoses.length > 0 ? (
                                    ...
                                    {canWrite && (
                                        <div style={{ display: 'flex', gap: '8px' }}>
                                            <Button
                                                type="text"
                                                icon={<FaEdit />}
                                                aria-label={t('edit', { defaultValue: 'Tahrirlash' })}
                                                title={t('edit', { defaultValue: 'Tahrirlash' })}
                                                onClick={() => { setEditingId(diag.id); setEditText(diag.diagnosisText); }}
                                                style={{ color: '#64748b' }}
                                            />
                                            <Popconfirm title={t('are_you_sure', { defaultValue: 'Ishonchingiz komilmi?' })} onConfirm={() => handleDelete(diag.id)}>
                                                <Button
                                                    type="text"
                                                    danger
                                                    icon={<FaTrash />}
                                                    aria-label={t('delete', { defaultValue: "O'chirish" })}
                                                    title={t('delete', { defaultValue: "O'chirish" })}
                                                />
                                            </Popconfirm>
                                        </div>
                                    )}
                    ...
                    canWrite && (
                        <div style={{ textAlign: 'center', padding: '20px 0', color: '#94a3b8' }}>
                            <p>{t('no_diagnosis_yet', { defaultValue: 'Hali tashxis yozilmagan' })}</p>
                        </div>
                    )
                )}
                ...
                {canWrite && !hasOwnDiagnosis && !editingId && (
                    <div style={{ background: '#f8fafc', padding: 16, borderRadius: 8, border: '1px solid #e2e8f0', marginTop: diagnoses.length > 0 ? 20 : 0 }}>
                        <Text strong style={{ display: 'block', marginBottom: 8 }}>{t('new_diagnosis', { defaultValue: 'Yangi tashxis:' })}</Text>
                        <TextArea 
                            rows={4} 
                            value={newText} 
                            onChange={e => setNewText(e.target.value)} 
                            placeholder={t('enter_diagnosis_text', { defaultValue: 'Shifokor tashxisini va tavsiyalarini kiriting...' })}
                            style={{ marginBottom: 16 }}
                        />
                        <div style={{ display: 'flex', gap: 12 }}>
                            <Button type="primary" loading={isSaving} onClick={handleSaveNew}>
