...
from models import HolterAnalyses
import datetime
from typing import Optional

def get_holter_analyse_by_id(session: Session, analyse_id: int) -> Optional[HolterAnalyses]:
    """
    Ma'lum ID bo'yicha HolterAnalyses yozuvini qaytaradi.
    Agar yozuv topilmasa, None qaytaradi.
    """
    return session.query(HolterAnalyses).filter(HolterAnalyses.id == analyse_id).first()


def create_holter_analyse(
    session: Session,
    patient_id: int,
    ...
    new_lab.document_number = f"NMED-HOL-{str(new_lab.id).zfill(8)}"
    session.commit()

    return new_lab


def update_holter_analyse(
    session: Session,
    analyse_id: int,
...
) -> Optional[HolterAnalyses]:
    """
    HolterAnalyses yozuvini yangilaydi.
    Agar yozuv topilmasa, None qaytaradi.
    AI tomonidan aniqlangan barcha parametrlar ham bazaga yoziladi.
    """

    lab = session.query(HolterAnalyses).filter(HolterAnalyses.id == analyse_id).first()
    if not lab:
        return None
