...
def update_ecg_analyse(session: Session, ecg_id: int,
                       ...
                       generated_short_file_link: bytes = None,
                       ai_answer_data: bytes = None,
                       clear_ai_answer: bool = False) -> ECGAnalyse | None:
    ecg = session.query(ECGAnalyse).filter(ECGAnalyse.id == ecg_id).first()
    if not ecg:
        return None
    ...
    # Aniq tozalash: yuqoridagi shartlar `None` ni "bu maydonga tegma"
    # deb qabul qiladi, shuning uchun o'chirish uchun alohida bayroq
    # kerak. Ilgari shu maqsadda bo'sh satr yozilardi, lekin ustun
    # `jsonb` ga o'tgach bo'sh satr yaroqsiz bo'lib qoldi (T-036).
    if clear_ai_answer:
        ecg.ai_answer_data = None
