...
from holter_analyse import get_holter_analyse_by_id, create_holter_analyse, update_holter_analyse
...
def save_analyse_file(file_bytes: bytes, filename: str) -> str:
    ...
    """
    return storage.save("holter", filename, file_bytes)
...
def _sync_holter_openai(content: bytes, fname: str, age: int, gender: str, lang: str) -> dict:
    ...
    resp = client.responses.create(
        ...
        **ai_config.diagnosis_request(),
        ...
        text=ai_schema.response_format("holter"),
    )
    try:
        return json.loads(resp.output_text)
    except Exception:
        logger.warning(
            "Holter: javobni JSON sifatida o'qib bo'lmadi, xom matn saqlanadi"
        )
...
async def _holter_ai_background(
    ...
    db = SessionLocal()
    try:
        # Fayl Holter tahliliga mos kelishini asosiy tahlildan oldin tekshirish.
        # Mos kelmasa qimmat tahlil umuman yuborilmaydi.
        mismatch = await asyncio.to_thread(
            classify_document, content, fname, "holter"
        )
        if mismatch:
            ...
            logger.info(
                "Holter: fayl turi mos emas (aniqlangan=%s), tahlil to'xtatildi: analyse_id=%d",
        ...
        ai_data_str = ai_result_guard.sanitize(ai_data_str, analyse_id, "holter")

        update_holter_analyse(session=db, analyse_id=analyse_id, status=STATUS_DONE, ai_answer_data=ai_data_str)
        logger.info("Holter AI muvaffaqiyatli: analyse_id=%d", analyse_id)

    except Exception as exc:
        logger.error("Holter AI fon xatolik analyse_id=%d [%s]: %s",
...
async def analyze(
    ...
    try:
        validate_upload(fname, content, "holter", lang)
    ...
    file_digest = duplicate_guard.check(
        db, "holter", patcient_id, content, force_duplicate
    )
    ...
    holter_analyse.file_hash = file_digest
    # Diskdagi nom UUID (T-099); asl nom saqlanadi va faylni
    # yuklab olishda foydalanuvchiga o'sha nom taklif qilinadi (T-101)
    holter_analyse.original_filename = (original_filename or first_file.filename or "")[:255] or None
    ...
    holter_analyse.ai_lang = (lang or 'uz')[:5]
    db.commit()
...
async def replace_file(
    ...
    """Mavjud Holter tahlilining faylini almashtirib, qayta tahlil qilish.
    ...
    analyse = get_holter_analyse_by_id(db, id)
    if not analyse:
        raise HTTPException(status_code=404, detail="Tahlil topilmadi")

    content = await file.read()
    fname = prepare_upload_filename(
        file.filename or "upload", content, default_stem="holter_upload"
    ).lower()
    ...
    task = asyncio.create_task(
        _holter_ai_background(id, content, fname, age, gender, lang)
    )
    _bg_tasks.add(task)
    task.add_done_callback(_bg_tasks.discard)

    logger.info("Holter: fayl almashtirildi va qayta tahlilga yuborildi: analyse_id=%d", id)
...
async def retry_analyse(
    ...
    logger.info("Holter: qayta tahlilga yuborildi: analyse_id=%d", id)

    return JSONResponse(content={"holter_id": id, "status": "processing"})
