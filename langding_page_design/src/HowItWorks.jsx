// NMED landing — How It Works (vertical timeline)
const { GlassCard: WGlassCard } = window.NMED;
    ...
    <section className="section grid-bg">
      <div className="container">
        <div className="head-center reveal">
          ...
          <h2 className="headline">4 qadamda boshlang</h2>
        </div>
        ...
        <div style={{ position: "relative", maxWidth: 880, margin: "0 auto" }}>
          ...
          <div className="col" style={{ gap: 36 }}>
            {TLSTEPS.map((s, i) => {
              const right = i % 2 === 0;
              return (
                <div key={i} className="reveal tl-row" data-d={(i % 2) + 1} style={{ position: "relative" }}>
                  <span className="tl-node" aria-hidden="true"></span>
                  {right ? <div className="hide-md"></div> : null}
                  <div className="tl-card" style={{ gridColumn: right ? 2 : 1, position: "relative" }}>
                    <WGlassCard radius="xl">
                      <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                        <span className="icon-chip"><i data-lucide={s.icon}></i></span>
                        <span style={{ fontFamily: "var(--font-display)", fontWeight: 700, fontSize: 42, color: "rgba(42,167,155,0.35)", lineHeight: 1 }}>{s.n}</span>
                      </div>
                      <h3 style={{ fontFamily: "var(--font-display)", fontWeight: 700, fontSize: 21, margin: "18px 0 8px" }}>{s.title}</h3>
                      <p style={{ color: "var(--text-secondary)", fontSize: 15, lineHeight: 1.6, margin: 0 }}>{s.body}</p>
                    </WGlassCard>
                  </div>
                  {!right ? <div className="hide-md"></div> : null}
                </div>
